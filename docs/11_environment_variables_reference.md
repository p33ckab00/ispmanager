...
- canonical base URL for public links generated by the app
...
- use the real public domain in production
- do not leave SMS/public links tied to localhost
...
- set to `True` in Ubuntu production behind HTTPS
...
- set to `True` when Nginx and TLS are active

### `SECURE_PROXY_SSL_HEADER`

Purpose:

- tells Django how the reverse proxy reports HTTPS
...
- set this when Nginx terminates HTTPS and proxies to Gunicorn

## 4. Scheduler Variables

These are important for production-safe scheduler separation.

### `DISABLE_SCHEDULER`

Purpose:

- prevents scheduler auto-start behavior in the web app process
...
- local development web process: `0` unless you intentionally want it disabled
- staging web service: `1`
- production web service: `1`

Operational guidance:

- run scheduled jobs through `python manage.py run_scheduler` as a separate service
- do not rely on Gunicorn workers to own APScheduler in production
...
- `DISABLE_SCHEDULER=1` for the web service
...
- support PostgreSQL env-based config
- support `APP_BASE_URL`
- support secure production defaults
- support dedicated scheduler service deployment
