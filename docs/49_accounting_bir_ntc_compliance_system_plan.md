...
The system must clearly distinguish whether ISP Manager is only helping with
internal accounting reports, producing loose-leaf books, or acting as a
BIR-registered invoicing or accounting system.

Supported modes should be explicit:

- internal books and management reports only
- loose-leaf books and print-ready book exports
- computerized books or CAS/CBA-assisted accounting mode
- electronic invoice or EIS-ready mode, if ever required

This distinction is important because generating internal worksheets is very
different from issuing BIR-valid invoices or operating as a registered
...
- export manifest and archive rules
- hash/checksum requirement for finalized exports
...
- accrual basis with optional cash-basis management views
- revenue recognition for prepaid and postpaid billing
- VAT treatment for uncollected receivables and unpaid payables
- depreciation method and useful life defaults
- capitalization threshold
- inventory costing method
- bad debt and write-off policy
- rounding and materiality policy
- treatment of penalties, discounts, waivers, and service credits

### Payment clearing and reconciliation settings

- default clearing accounts for GCash, Maya, bank transfer, cash, and gateways
- settlement batch rules
- payment gateway fee mapping
- failed, reversed, duplicate, and chargeback handling
- bank reconciliation rules

### Compliance calendar settings

- BIR monthly, quarterly, and annual task templates
- NTC quarterly, annual, renewal, and fee task templates
- due date rules
- reminder rules
- filed date and confirmation reference tracking
- attachment checklist per task
...
- report template version per NTC office or permit type
- supervision, regulation, annual, renewal, and related fee tracking
...
### BIR form mapping registry

The eBIRForms guide should be powered by a versioned mapping registry, not
hard-coded report text.

Each mapping should define:

- form name
- form version or effective date
- return period type
- item or line number
- field label
- formula
- source ledger, source schedule, or source query
- validation rule
- accountant override and review note, if permitted
- mapping status: draft, active, retired

This allows the system to keep old period reports reproducible even after BIR
forms or instructions change.

### BIR attachment and schedule support

The system should support schedules that may be needed depending on the
...
- annual inventory list for CPE, network materials, and supplies
...
### Loose-leaf package controls

Loose-leaf book exports need more than just rows. Each finalized export should
include:

- cover sheet per book
- taxpayer profile details
- book title
- period covered
- page range and total page count
- generated by and generated timestamp
- reviewed by and reviewed timestamp, if reviewed
- export manifest listing every included file
- hash or checksum per file
- archived copy of the exact generated package
- affidavit helper values such as book names, page counts, and period covered

Once a loose-leaf package is finalized, the exact exported package should remain
immutable. Later corrections should create an amendment or adjustment package,
not silently overwrite the filed package.

### BIR/CAS/e-invoicing risk gate

If ISP Manager issues BIR-valid invoices or becomes the official accounting
books system, the operator may need to treat it differently from a management
reporting tool. The product must therefore include a compliance mode gate.

The UI should warn when a configuration moves from:

```text
management reporting only
-> loose-leaf book support
-> computerized books/CAS/CBA support
-> electronic invoice or EIS-ready support
```

Exact implementation requirements should be verified with the taxpayer's
accountant and the current BIR rules before enabling official invoicing or
computerized books behavior.
...
- filed reference and attachment tracker
...
### NTC template registry

NTC report layouts can vary by permit type, region, office, and current
instruction. The system should not hard-code only one universal format.

The NTC compliance module should support:

- report template version
- source fields and formulas
- period type
- permit or service type applicability
- required signatories
- required attachment checklist
- filed date and receiving reference
- renewal or annual report linkage
- fee or assessment linkage

If a regional office provides a specific spreadsheet or prescribed form, the
operator should be able to store that template reference and map ISP Manager
data into it.
...
The NTC pack should also include a filing checklist and archive entry so the
operator can later prove which files were prepared, reviewed, and submitted.
...
## 15. Opening Balances and Migration

The current system already has billing, payments, credits, income, and expense
records. Accounting v2 needs a formal cutover workflow before it becomes the
source of truth.

The cutover workflow should capture:

- opening trial balance
- AR opening balance per subscriber
- customer credits and advances
- subscriber deposits
- unpaid vendor bills and accrued expenses
- bank, cash, GCash, Maya, and payment gateway balances
- CPE, ONU, network material, and spare inventory count
- fixed asset opening balances and accumulated depreciation
- unpaid taxes and withholding balances
- loans and other liabilities
- retained earnings or owner equity opening amount

It should generate a migration reconciliation report:

```text
Billing AR total
-> subscriber AR subledger total
-> GL Accounts Receivable control account
```

No production cutover should proceed until control accounts reconcile with their
subledgers.

## 16. Bank Reconciliation and Payment Clearing

The payment flow must support real-world payment channels, not only immediate
cash receipt.
...
- GCash and Maya clearing reconciliation
...
The system should keep unmatched, partially matched, and reconciled statuses.

## 17. Approval, Filing, and Amendment Workflow

Accounting records and compliance reports need explicit lifecycle states.
...
Period closing should prevent normal edits after lock. Corrections should use
reversal entries, adjustment entries, amendment exports, or reopened periods
with audit approval.

Compliance report packs should track:
...
- filed date
- filed channel
- receiving reference or confirmation number
- attachment list
- archive hash

## 18. Candidate Data Models
...
BirFormMapping
BirAttachmentSchedule
NtcReportTemplate
CompliancePackageArchive
...
- lifecycle status
- filed date and filed reference
- file hash/checksum
- source data snapshot reference

## 19. Implementation Phases

### Phase 0 - Compliance mode and migration planning

- define accounting compliance mode
- define opening balance workflow
- define data migration and reconciliation rules
- identify which legacy income, expense, payment, invoice, and credit records
  should be bridged into Accounting v2
- define initial tenant/accounting entity creation
...
- add lifecycle statuses for draft, posted, reviewed, locked, filed, amended,
  reversed, and voided records
- add compliance calendar scaffolding
...
- add accounting policy defaults for each template
...
- VAT treatment for uncollected receivables and unpaid payables where applicable

### Phase 4 - Expenses, vendors, inventory, assets, and clearing
...
- bank, GCash, Maya, and payment gateway reconciliation
- payment gateway fee posting
...
- AR and AP control account reconciliation
...
- NTC report template registry
- filing checklist and fee tracker
- filed reference and archive tracking
...
- filed package amendment workflow
- full accounting and compliance test matrix
...
- compliance mode is explicit and warns before official invoicing/CAS behavior
- opening balances can be imported and reconciled
- AR aging equals the GL receivables control account
- AP aging equals the GL payables control account
- VAT schedules reconcile to the VAT ledger
- cash, bank, wallet, and gateway clearing can be reconciled
- loose-leaf exports include manifest, page counts, and immutable archive hashes
- BIR form guide mappings are versioned
- NTC report templates are versioned or configurable
- compliance calendar tasks can track due, prepared, reviewed, filed, and
  amended statuses

### Required test scenarios

- VAT invoice with full payment
- VAT invoice with partial payment
- non-VAT invoice and percentage tax schedule
- advance payment before invoice generation
...
- payment gateway settlement with fees
- reversed or duplicate payment
- period lock and reversal adjustment
- trial balance debits equal credits
- AR subledger equals GL AR control account
- VAT schedule equals VAT ledger
- exported loose-leaf pack can be regenerated or verified by archive hash

## 21. Open Inputs Needed Before Build
...
- subscriber AR and credit listing at cutover
- CPE, inventory, and fixed asset listings
- bank, wallet, gateway, and cash balances at cutover
- preferred accounting policies for depreciation, inventory, capitalization, bad
  debts, and revenue recognition
- filing calendars and renewal dates
- existing loose-leaf, CBA/CAS, or invoicing approvals, if any

The system should still provide defaults so the operator is not blocked while
waiting for the accountant.

## 22. Official Reference Points Checked
...
- BIR electronic invoicing references, including RR 8-2022 and RR 11-2025
  https://bir-cdn.bir.gov.ph/local/pdf/RR%208-2022.pdf
  https://bir-cdn.bir.gov.ph/BIR/pdf/RR%2011-2025.pdf
