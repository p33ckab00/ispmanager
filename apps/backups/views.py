...
def dashboard(request):
    ...
    latest_success = BackupJob.objects.filter(status='completed').order_by('-completed_at', '-created_at').first()
    latest_failure = BackupJob.objects.filter(status='failed').order_by('-completed_at', '-created_at').first()
